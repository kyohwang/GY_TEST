...
public class Torrent {
	...
	public Torrent(byte[] torrent) throws IOException {
		...
		for(String s : this.decoded.keySet()){
		...
		List<BEValue> list = this.decoded.containsKey("nodes")
				? this.decoded.get("nodes").getList()
				: null;
				
		this.nodes = new HashMap<String,Integer>();
		if(list != null){
			for(BEValue b : list){
				List<BEValue> ls = b.getList();
				this.nodes.put(ls.get(0).getString(), ls.get(1).getInt());
			}
		}
